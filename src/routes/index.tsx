...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Aufgaben der Bauplanung mit Status, Unternehmen und Anhängen.",
      },
    ],
  }),
  component: TasksPage,
...
function TasksPage() {
  ...
  async function handleMove(
    ...
    const walk = (pid: string) => {
      for (const c of childrenByParentId.get(pid) ?? []) {
        descendants.push(c);
        walk(c.id);
      }
    };
    ...
    if (
      newParentId &&
      (newParentId === draggedId || descendants.some((d) => d.id === newParentId))
    ) {
    ...
    toast.success(
      newParent
        ? `Verschoben unter „${newParent.title || "Aufgabe"}"`
        : "Auf oberste Ebene verschoben",
    );
  ...
  async function handleCreateAndEdit() {
    const maxOrder = tasks.reduce(
      (m, t) => (t.parent_id === null ? Math.max(m, t.sort_order) : m),
      0,
    );
    const { data, error } = await supabase
      .from("tasks")
        ...
        <div className="px-4 py-3 flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <HelpButton title="Aufgabenliste">
                ...
                <ul className="list-disc pl-4 space-y-1">
                  <li>
                    <b>Tippen</b> auf eine Zeile: Detailansicht öffnen (dort gibt es alle Aktionen)
                  </li>
                  <li>
                    <b>Tippen</b> auf das Status-Symbol: Status weiterschalten
                  </li>
                  <li>
                    <b>Pfeil</b> rechts an Eltern-Aufgaben: Unteraufgaben ein-/ausblenden
                  </li>
            ...
            <p className="text-xs text-muted-foreground">
              {filtered.length}
              {filterActive ? ` / ${ordered.length}` : ""} Aufgaben ·{" "}
          ...
          <div className="flex gap-1.5 overflow-x-auto -mx-1 px-1 pb-0.5">
            ...
            {filterActive && (
              <button
                ...
                className="shrink-0 text-xs text-muted-foreground underline-offset-2 hover:underline px-2"
              >
                zurücksetzen
              </button>
            )}
          ...
          <li key={t.id}>
            <TaskRow
              ...
              onCycleStatus={() =>
                applyUpdate(
                  t.id,
                  { status: nextStatus(t.status, statusMeta.order) },
                  `Status: „${t.title || "Aufgabe"}"`,
                )
              }
              ...
              pointerDropPosition={
                pointerDrag?.dropTargetId === t.id ? pointerDrag.dropPosition : null
              }
