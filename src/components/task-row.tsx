...
export function TaskRow({
  ...
  return (
    ...
    >
      {effectiveDropPos === "before" && (
        <div className="absolute left-0 right-0 -top-px h-0.5 bg-primary z-10 pointer-events-none" />
      )}
      {effectiveDropPos === "after" && (
        <div className="absolute left-0 right-0 -bottom-px h-0.5 bg-primary z-10 pointer-events-none" />
      )}
      <div
        className={`border-b bg-card ${effectiveDropPos === "child" ? "ring-2 ring-inset ring-primary/60" : ""}`}
      >
        <div className="relative">
          ...
          {/* Mobile compact row */}
          <div
            className="flex md:hidden items-center gap-2 py-2.5 pr-2"
            style={{ paddingLeft: 12 + task.depth * 16 }}
          >
            <span
            ...
            {hasChildren && (
              <button
                ...
                aria-label={
                  childrenCollapsed ? "Unteraufgaben einblenden" : "Unteraufgaben ausblenden"
                }
              >
                <ChevronRight
